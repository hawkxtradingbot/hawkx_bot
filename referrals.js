...
async function runDailyPayout(notifyCallback) {
  ...
  for (const payout of payouts) {
    ...
    console.log(`[Payout] DEVNET: User ${payout.user_id} → ${payout.total.toFixed(6)} SOL (simulated)`);
    if (notifyCallback) {
      notifyCallback(payout.user_id, 'DAILY_PAYOUT', { sol: payout.total.toFixed(4) });
    }
  }
}
...
function getReferralStats(userId) {
  ...
  const allTime = db.getDb().prepare('SELECT SUM(earned_sol) as total FROM referral_earnings WHERE user_id = ?').get(userId);
  return {
    chain,
...
module.exports = { creditReferralEarnings, runDailyPayout, startDailyPayoutCron, getReferralStats };
