...
function getFeeRate(user) {
  ...
  if (user.rank === 1 && user.joiner_discount) return config.FEE_RATES.scoutReferral;
  const rankKey = config.RANK_FEE_KEYS[user.rank] || 'scout';
  return config.FEE_RATES[rankKey];
}

function getRankName(rankNum, lang = 'en') {
  const keys = { 1:'rank.scout', 2:'rank.tracker', 3:'rank.hunter',
    4:'rank.predator', 5:'rank.apex', 6:'rank.hawk', 7:'rank.hawkelite' };
  ...
  if (newRank > user.rank) {
    ...
    if (notifyCallback) {
      ...
      notifyCallback(userId, 'RANK_UP', {
        rank: rankName, num: newRank,
        volume: user.cumulative_volume_sol.toFixed(2),
        fee, tagline: '', username: user.username || 'Trader',
      });
    }
    return true;
  }
  ...
  cron.schedule('* * * * *', async () => {
    ...
    for (const user of users) {
      ...
      if (daysSince >= 7) {
        ...
        try {
          await bot.api.sendMessage(user.user_id, t('trial.expired', user.language), { parse_mode: 'Markdown' });
        } catch {}
      }
    }
  });
...
module.exports = { getFeeRate, getRankName, checkAndPromote, startTrialCron, startRankCron };
