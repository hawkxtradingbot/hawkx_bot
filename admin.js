...
function isAdmin(userId) { return config.ADMIN_IDS.includes(String(userId)); }
...
async function showAdminPanel(ctx) {
  ...
  await ctx.reply(msg, {
    ...
    reply_markup: {
      inline_keyboard: [
        [{ text: ks ? '✅ Resume' : '🔴 Kill-Switch ON', callback_data: 'admin_killswitch' }],
...
async function handleAdminCallback(ctx, action) {
  ...
  if (action === 'admin_killswitch') {
    ...
    await ctx.answerCallbackQuery(killSwitch.isActive() ? '🔴 Kill-switch ON' : '✅ Resumed');
    ...
    await ctx.answerCallbackQuery(`Simulated 0.5 SOL volume for ${users.length} users`);
  }
}
