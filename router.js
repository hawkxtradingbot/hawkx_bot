...
const { showSettings, handleSettingCallback, handleTextInput } = require('./settings');
const { buildMainMenu, buildTradeMenu, buildModulesMenu, buildWalletMenu } = require('./keyboards');
...
async function setupRouter(bot) {
  ...
  bot.command('trade', async (ctx) => {
    ...
    const user = await ensureUser(ctx); if (!user) return;
  ...
  bot.command('settings', async (ctx) => {
    ...
    await ctx.reply('⚙️ *Settings*', {
      parse_mode: 'Markdown',
      reply_markup: require('./keyboards').buildSettingsMenu({ ...user, settings }),
    });
  });
  ...
  bot.command('wallets', async (ctx) => {
    ...
    if (!wallets.length) {
      await ctx.reply('No wallets yet.', {
        reply_markup: { inline_keyboard: [[{ text: '🆕 Generate Wallet', callback_data: 'wallet_generate' }]] },
      });
      return;
    }
  ...
  bot.command('mockbuy', async (ctx) => {
    const user = await ensureUser(ctx); if (!user) return;
    if (killSwitch.isActive()) return ctx.reply(t('killswitch.active', user.language));
  ...
  bot.command('addvolume', async (ctx) => {
    ...
    const promoted = checkAndPromote(user.user_id, require('./notifications').notify);
  ...
  bot.command('mystats', async (ctx) => {
    ...
    const fee = user.trial_active ? 0.3 : (config.FEE_RATES[rankKey] * 100).toFixed(2);
    const nextRank = user.rank < 7 ? config.RANK_THRESHOLDS[user.rank + 1] : 'MAX';
    const needed = user.rank < 7 ? (nextRank - user.cumulative_volume_sol).toFixed(2) : '—';
  ...
  bot.on('callback_query:data', async (ctx) => {
    ...
    if ((data.startsWith('trade_') || data.startsWith('devnet_mock')) && killSwitch.isActive()) {
      await ctx.answerCallbackQuery(t('killswitch.active', user?.language || 'en'));
      return;
    }
    ...
    try {
      ...
      if (data === 'menu_main') {
        ...
        try {
          await ctx.editMessageText('🦅 *HawkX* [DEVNET]', {
            parse_mode: 'Markdown', reply_markup: buildMainMenu(fresh),
          });
        } catch {
        ...
        await safeEdit(ctx, '📦 *Modules* — All features at all ranks:', buildModulesMenu());
        ...
        await safeEdit(ctx, '👛 *Wallets*', buildWalletMenu(wallets, user?.active_wallet_id));
      ...
      // ── WALLET ACTIONS ──
        ...
        await ctx.reply('📥 Send your base58 private key:');
      ...
      // ── DEVNET TEST BUTTONS ──
        ...
        await ctx.answerCallbackQuery(`+1 SOL volume. Total: ${updated.cumulative_volume_sol.toFixed(2)} SOL`);
      ...
      try { await ctx.answerCallbackQuery('Error!'); } catch {}
    }
  });
  ...
  bot.on('message:text', async (ctx) => {
    ...
    if (!user) { await handleStart(ctx); return; }
...
async function ensureUser(ctx) {
  const user = db.getUser(ctx.from.id);
  if (!user) { await handleStart(ctx); return null; }
  return user;
}
