...
function setupRouter(bot) {
  ...
  bot.on("callback_query:data", async (ctx) => {
    ...
    if (data === "mode_set_pro") {
      ...
      return safeEdit(
        ctx,
        `🦅 *HawkX* [DEVNET] — ⚡ Pro Mode\n\n${getGuide("main_pro")}`,
        buildMainMenu(
          freshUser,
          db.getTodayStats(userId, db.getUser(userId).active_wallet_id),
          ks,
        ),
      );
    }
    ...
    if (data === "mode_set_beginner") {
      ...
      return safeEdit(
        ctx,
        `🦅 *HawkX* [DEVNET] — 🌱 Beginner Mode\n\n${getGuide("main_beginner")}`,
        buildMainMenu(
          freshUser,
          db.getTodayStats(userId, db.getUser(userId).active_wallet_id),
          ks,
    ...
    if (
      ...
      data.startsWith("bset_") ||
      data.startsWith("pset_") ||
      data.startsWith("sap_") ||
      data.startsWith("alert_") ||
      data.startsWith("ast_") ||
      data.startsWith("ast_select_") ||
      data.startsWith("ast_view_") ||
      data.startsWith("ast_back_") ||
      data.startsWith("ab_") ||
      data.startsWith("sas_") ||
      data === "pset_autosell_manual" || 
      data === "pset_autosell_screen" ||
      data === "pset_autobuy_screen"
    ) {
      if (data.startsWith("lang_")) {
        const lang = data.replace("lang_", "");
        idb.updateUser(userId, { language: lang });
    ...
    if (data.startsWith("buy_ca_amt_")) {
      ...
      if (!ca) {
        await ctx.answerCallbackQuery("❌ Please paste a token CA first.");
        return;
      }
      await ctx.answerCallbackQuery();
    ...
    if (data === "sell_initial" || data.startsWith("sell_initial_")) {
      ...
      await ctx.answerCallbackQuery();
      const posId =
        data.includes("_") && data !== "sell_initial"
          ? parseInt(data.split("_")[2])
          : null;
      const positions = posId
        ? [db.getPosition(posId, userId)]
        : db.getOpenPositions(userId);
      ...
      const initialPct =
        currentValue > 0
          ? Math.min(100, (pos.sol_invested / currentValue) * 100)
          : 100;
...
${getGuide("sniper")}`, buildSniperMainMenu());
    ...
    if (data.startsWith("snipe_view_")) {
      ...
      if (!s) { await ctx.answerCallbackQuery("Not found."); return; }
      await ctx.answerCallbackQuery();
