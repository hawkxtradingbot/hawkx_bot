  ...
  try {
    ...
    if (cardResult && cardResult.type === "photo") {
      ...
      db.setSysConfig(`last_card_data_${pos.user_id}`, JSON.stringify({
        ...
        feeRate: rank.fee,
        _pnlSol: pnlSolVal, _pnlUsd: Math.abs(pnlSolVal * (global.__hawkxSolPx || 150)),
...
async function checkDcaOrders(notifyFn) {
  ...
  for (const o of orders) {
    try {
      ...
      try {
        if (o.wallet_id && o.wallet_id !== prevActive) {
        ...
        if (o.wallet_id && o.wallet_id !== prevActive) {
          db.getDb().prepare("UPDATE users SET active_wallet_id = ? WHERE user_id = ?").run(prevActive, o.user_id);
        }
      }
